...
void botTai();
int minimax(int boardd, int kedalaman, bool isMaximizing);
void gameBoard();
int playerWin();
int botWin();
int isFull();
...
//kedalaman untuk melihat berapa kali perulangan
...
//inisialisasi bestscore sebagai -1000 karena kita mau dapat maximum saat bot jalan
		...
		for(int i=0; i<9; i++){
			if(board[i]=='*'){
				board[i] = 'O';
				int score = minimax(i, kedalaman+1, false);
				board[i] = '*';
				if(score > bestScore){
					bestScore = score;
				}
...
//inisialisasi bestscore sebagai 1000 karena kita mau dapat maximum saat player jalan
		...
		for(int i=0; i<9; i++){
			if(board[i]=='*'){
				board[i] = 'X';
				int score = minimax(i, kedalaman+1, true);
				board[i] = '*';
				if(score < bestScore){
					bestScore = score;
				}
